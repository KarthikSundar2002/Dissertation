...
class MLP(nn.Module):
    def __init__(self, hidden_size: int = 128, hidden_layers: int = 3, emb_size: int = 128,
        ...
        layers.append(nn.Linear(hidden_size, attn_size))
        layers.append(nn.GELU())
        layers.append(nn.LayerNorm(attn_size))
        self.attn = nn.MultiheadAttention(embed_dim=attn_size, num_heads=4)
        post_attn_layers = [nn.GELU(), nn.Linear(attn_size, hidden_size), nn.Linear(hidden_size, self.num_embeddings)]
        self.post_attn = nn.Sequential(*post_attn_layers)
    ...
    def forward(self, x,t,x_mask, y):
       ...
       # print(f"t: {t}")
        ...
        t_emb = t_emb.repeat(1,x_emb[0].shape[1],1)
