...
from networks.flowmatching.flow_one_stage import SRM as srm
#from networks.flowmatching.set_transformer import SetTransformer
from networks.flowmatching.model import Model
import torch
...
experiment_name = ' SRM Set Latent 1024 Dim MLP 10k No Mask'
...
srm = srm(model, experiment_name, samples, sample_steps, format_path, size,dim_in, learning_rate)
...
#srm.load_state_dict(torch.load("srm.ckpt", weights_only=False)["state_dict"])
#srm = torch.compile(srm)
trainer = L.Trainer(accelerator='gpu', devices=gpu_num, strategy='auto' ,logger=wandb_logger, max_epochs=500,
